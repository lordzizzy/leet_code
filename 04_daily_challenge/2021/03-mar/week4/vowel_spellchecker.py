...
class Solution:
    def spellchecker(self, wordlist: List[str], queries: List[str]) -> List[str]:
        return self.spellchecker_dict_elegant(wordlist, queries)

    def spellchecker_dict_elegant(
    ...
    # TODO
    # https://leetcode.com/problems/vowel-spellchecker/discuss/388977/Very-Easy-Python-Solution-or-Beats-97-Time-100-Space


SolutionFunc = Callable[[List[str], List[str]], List[str]]


def test_solution(wordlist: List[str], queries: List[str], expected: List[str]) -> None:
    def test_impl(
        func: SolutionFunc, wordlist: List[str], queries: List[str], expected: List[str]
    ) -> None:
        r = func(wordlist, queries)
        if r == expected:
            print(
                colored(
                    f"PASSED {func.__name__} => {wordlist} with spellcheck queries: {queries} = {r}",
                    "green",
                )
            )
        else:
            print(
                colored(
                    f"FAILED {func.__name__} => {wordlist} with spellcheck queries: {queries} = {r} but expected {expected}",
                    "red",
                )
            )

    sln = Solution()
    test_impl(sln.spellchecker_dict_elegant, wordlist, queries, expected)
