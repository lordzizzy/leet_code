...
class Solution:
    ...
    def randPoint(self) -> List[float]:
        ...
        rand_radius = math.sqrt(random.random()) * self.radius
        x = self.x_center + math.cos(rand_angle) * rand_radius
        y = self.y_center + math.sin(rand_angle) * rand_radius
        return [x, y]


def test_solution(radius: float, x_center: float, y_center: float) -> None:
    obj = Solution(radius, x_center, y_center)
    param_1 = obj.randPoint()


if __name__ == "__main__":
    # test_solution(radius=)
    pass
