...
class UndergroundSystem(Protocol):
    def __init__(self):
        ...
...
class UndergroundSystem_impl:
    def __init__(self):
        # maps id -> (station name, board time)
        self.check_in: Dict[int, Tuple[str, int]] = {}
        # maps (start station name, end station name) -> (total duration,
        # journey count)
        self.time: Dict[Tuple[str, str], Tuple[int, int]] = {}
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        if id in self.check_in:
            start_station, board_time = self.check_in[id]
            trip_duration = t - board_time
            station_pair = (start_station, stationName)
            if station_pair not in self.time:
                self.time[station_pair] = (trip_duration, 1)
            else:
                total_duration, journey_count = self.time[station_pair]
                self.time[station_pair] = (
                    total_duration + trip_duration,
                    journey_count + 1,
                )

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        total_duration, journey_count = self.time[(startStation, endStation)]
        return total_duration / journey_count
