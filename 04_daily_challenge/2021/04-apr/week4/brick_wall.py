...
class Solution:
    def leastBricks(self, wall: List[List[int]]) -> int:
        dp = DefaultDict[int, int](lambda: 0)       
        maxEdges = 0

        for row in wall:
            for idx in accumulate(row[:-1]):
                dp[idx] += 1
                maxEdges = max(maxEdges, dp[idx])
...
def test_solution(wall: List[List[int]], expected: int) -> None:
    def test_impl(func: SolutionFunc, wall: List[List[int]], expected: int) -> None:
        r = func(wall)
        if r == expected:
            print(
                colored(
                    f"PASSED {func.__name__} => Least bricks path of {wall} is {r}",
                    "green",
                )
            )
        else:
            print(
                colored(
                    f"FAILED {func.__name__} => Least bricks path of {wall} is {r}, but expected {expected}",
                    "red",
                )
            )

    sln = Solution()
    test_impl(sln.leastBricks, wall, expected)
